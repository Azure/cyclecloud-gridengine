...
def from_env():
    ...
    return from_config(sge_root, sge_admin, nodename)


def from_config(sge_root, gridengine_user_name, nodename):
    return """#-------------------------------------------------
...
ADMIN_USER="%(gridengine_user_name)s"

# The dir, where qmaster spools this parts, which are not spooled by DB
#(mandatory for qmaster installation)
QMASTER_SPOOL_DIR="%(gridengineroot)s/default/spool/qmaster"
...
# directory
#(mandatory for qmaster installation)
EXECD_SPOOL_DIR="%(gridengineroot)s/default/spool"
...
DB_SPOOLING_DIR="%(gridengineroot)s/spooldb"
...
# any entry
ADMIN_HOST_LIST="%(nodename)s"

# A List of Host which should become submit hosts
# If you do not enter any host here, you have to add all of your hosts
# by hand, after the installation. The autoinstallation works without
# any entry
SUBMIT_HOST_LIST="%(nodename)s"
...
""" % ({"gridengineroot": sge_root,
        "gridengine_user_name": gridengine_user_name,
        "nodename": nodename
        })
