...
def _build_placement_groups(hosts_to_reqs):
    ...
    for host, reqs_dict in hosts_to_reqs.iteritems():
...
def assign_job_to_pg(job, pg_set, groups_enabled, max_group_backlog=1):
    ...
    if slot_type is None:
        slot_type = 'execute'
        logging.debug("Assigning default slot_type (execute) to job_number %s", job["job_number"])
...
def main():
    ...
    for job in all_jobs.itervalues():
    ...
    for job in (already_assigned_jobs + unassigned_jobs):
        ...
        if updates_to_apply:
            ...
            command = ""
            for k, v in job['hard_resources'].iteritems():
